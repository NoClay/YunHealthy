...
public class LoginActivityPresenter extends BasePresenter<LoginActivityInterface> {
            ...
            public void done(List<SignUserData> list, BmobException e) {
                if (e == null){
                    if (list == null){
                        ...
                        user = list.get(0);
                        getView().loadImage(user.getUserImage().getUrl().toString());
                    }
                }else{
                    e.printStackTrace();
                    getView().loadImage(null);
    ...
    public void loginIfRemember(){
        Log.d("test", "zhixingle");

        SharedPreferences sharedPreferences =
                MyApplication.getContext().getSharedPreferences("LoginState", MODE_PRIVATE);
        Log.d("test", "loginIfRemember: state = " + sharedPreferences.toString());
        boolean isRemember = sharedPreferences.getBoolean("loginRememberState", false);
        if (isRemember){
            getView().loginSuccess();
        }
    }
