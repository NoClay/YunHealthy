...
unsigned char Com3RecArr[RAS] = {0};//接收串口数据的数组，请按实际需要修改大小

//这几位是不是也学他的编写方式？
//bit ComFlag = 0;//接收数据标志位，也可表示产生了数据收发
unsigned char Com3RecComplete = 0;//数据接收成功的标志位，接收完成后需手动置零
...
void USART3_Init(unsigned int pclk1, unsigned int bound)
{
	...
	RCC->APB2ENR |= 0x00000008;//使能PORTB时钟
	GPIOB->CRH &= 0xFFFF00FF;
	GPIOB->CRH |= 0x00008B00;

	RCC->APB1ENR |= 0x00040000;//使能串口3时钟
	RCC->APB1RSTR |= 0x00040000;//复位串口3
	RCC->APB1RSTR &= 0xFFFBFFFF;//停止复位串口3
	...
	USART3->BRR = mantissa;//这样设置的波特率精度更高
	
	USART3->CR1 |= 0x0000200C;//8位数据，无校验位，启用收发使能
	//CR2、CR3采用默认设置，
	...
	USART3->CR1 |= 0x00000100;//PE中断使能
	USART3->CR1 |= 0x00000020;//接收缓冲区非空中断使能
	MY_NVIC_Init(2, 3, USART3_IRQn, 2);//抢占3，子优先级3，组2。这样设置才应该是最低优先级吧？！
...
void USART3_IRQHandler(void)//串口中断服务，有校验
{
	static unsigned char Com3RecCount = 0;//数据接收计数器
	static unsigned char Com3CheckSum = 0;//求和校验
	...
	else if(Com3RecCount >= 2 && Com3RecCount < RAS - 1)//数据部分，长度按情况修改，
	{
		Com3RecCount++;
	}

	//有校验位
	else if(Com3RecCount == RAS - 1 && Com3RecArr[Com3RecCount] == Com3CheckSum - Com3RecArr[Com3RecCount])//我是在前面先求和的，所以这里要减
	{
